...
-   Dublin Core can be thought of as the cornerstone of Darwin Core. Dublin Core was created in 1994 as a simple, set of terms for describing web resources. Standards like Darwin Core built on it and continue to use Dublin Core terms, for example `language` to describe the language that a record is in. Because Dublin Core provides a simple, universal set of metadata elements (like title, creator, subject, date, identifier), that work across all kinds of digital resources, it’s widely adopted and shows up in libraries (to describe books and digital texts), museums (for artifacts and specimens), and across the internet (in data repositories, archives, and search engines). So why should you be aware of it? As a biologist you may use it as part of Darwin Core or when you dive deep into your library stacks.
...
The [THREDDS](https://www.unidata.ucar.edu/software/tds/) server, is a data service, like ERDDAP. THREDDS is closely tied to OPeNDAP, using it as one of its core protocols for serving scientific data, though it also supports others like WMS, WCS, and NetCDF Subset Service. ERDDAP, on the other hand, is more flexible: it can expose data through OPeNDAP but doesn’t depend on it, instead offering multiple access formats such as CSV, JSON, and custom REST-like APIs.
...
-   OGC WMS Standard; [Official specification and protocol details](https://www.ogc.org/standards/wms) 

- QGIS WMS Tutorial; [Hands-on lesson loading WMS layers](https://docs.qgis.org/latest/en/docs/training_manual/online_resources/wms.html
)

- GeoServer WMS Guide; [Practical WMS setup and usage](https://geoserver.geosolutionsgroup.com/edu/en/pretty_maps/wms.html
)

- Penn State GEOG 585; [Course module on web mapping systems](https://www.e-education.psu.edu/geog585/node/683
)

- GISGeography WMS Intro; [Clear overview of concepts and uses](https://gisgeography.com/web-mapping-services-wms/)
